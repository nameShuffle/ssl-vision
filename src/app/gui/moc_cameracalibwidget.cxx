/****************************************************************************
** Meta object code from reading C++ file 'cameracalibwidget.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "cameracalibwidget.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'cameracalibwidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_CameraCalibrationWidget[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      25,   24,   24,   24, 0x0a,
      60,   24,   24,   24, 0x0a,
      81,   24,   24,   24, 0x0a,
      99,   24,   24,   24, 0x0a,
     118,   24,   24,   24, 0x0a,
     141,  137,   24,   24, 0x0a,
     174,  137,   24,   24, 0x0a,
     205,  137,   24,   24, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_CameraCalibrationWidget[] = {
    "CameraCalibrationWidget\0\0"
    "is_clicked_update_control_points()\0"
    "is_clicked_initial()\0is_clicked_full()\0"
    "is_clicked_reset()\0edges_is_clicked()\0"
    "val\0cameraheight_slider_changed(int)\0"
    "distortion_slider_changed(int)\0"
    "line_search_slider_changed(int)\0"
};

void CameraCalibrationWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        CameraCalibrationWidget *_t = static_cast<CameraCalibrationWidget *>(_o);
        switch (_id) {
        case 0: _t->is_clicked_update_control_points(); break;
        case 1: _t->is_clicked_initial(); break;
        case 2: _t->is_clicked_full(); break;
        case 3: _t->is_clicked_reset(); break;
        case 4: _t->edges_is_clicked(); break;
        case 5: _t->cameraheight_slider_changed((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 6: _t->distortion_slider_changed((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 7: _t->line_search_slider_changed((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData CameraCalibrationWidget::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject CameraCalibrationWidget::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_CameraCalibrationWidget,
      qt_meta_data_CameraCalibrationWidget, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &CameraCalibrationWidget::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *CameraCalibrationWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *CameraCalibrationWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_CameraCalibrationWidget))
        return static_cast<void*>(const_cast< CameraCalibrationWidget*>(this));
    return QWidget::qt_metacast(_clname);
}

int CameraCalibrationWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
